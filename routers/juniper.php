...
final class Juniper extends Router {
	...
	protected function build_commands($command, $parameter) {
		...
		switch ($command) {
			case 'bgp' :
				if (match_ipv4 ( $parameter, false ) || match_ipv6 ( $parameter, false )) {
					$commands [] = 'show route ' . $parameter . ' protocol bgp protocol bgp all detail';
				} else {
			...
			case 'route' :
				if (match_ipv4 ( $parameter, false )) {
					$commands [] = 'show route ' . $parameter . ' active-path';
			...
			case 'as-path-regex' :
				if (match_aspath_regex ( $parameter )) {
					$commands [] = 'show route aspath-regex "' . $parameter . '" protocol bgp table inet.0';
					$commands [] = 'show route aspath-regex "' . $parameter . '" protocol bgp table inet6.0';
				} else {
