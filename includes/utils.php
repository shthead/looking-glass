 ...
 * @param string $ip
 *        	the parameter to test.
 * @param boolean $ip_only
 *        	optional parameter, if omitted or set to true, it
...
function match_ipv4($ip, $ip_only = true) {
	...
	if (strrpos ( $ip, '/' ) && ! $ip_only) {
		...
		if (! $config ['misc'] ['allow_private_ip'] && match_private_ip_range ( $ip_and_mask [0] )) {
			return false;
		}
		
		if (! $config ['misc'] ['allow_reserved_ip'] && match_reserved_ip_range ( $ip_and_mask [0] )) {
			return false;
		}
		
		return filter_var ( $ip_and_mask [0], FILTER_VALIDATE_IP, FILTER_FLAG_IPV4 ) && filter_var ( $ip_and_mask [1], FILTER_VALIDATE_INT );
	} else {
		if (! $config ['misc'] ['allow_private_ip'] && match_private_ip_range ( $ip )) {
			return false;
		}
		
		if (! $config ['misc'] ['allow_reserved_ip'] && match_reserved_ip_range ( $ip )) {
			return false;
		}
...
function match_as($as) {
	...
	if (filter_var ( $as, FILTER_VALIDATE_INT, $options_16bit_private )) {
		return isset ( $config ['misc'] ['allow_private_asn'] ) && $config ['misc'] ['allow_private_asn'] == true;
	}
 ...
 *
 * @param string $hostname
 *        	the hostname to use to search in the DNS
 *        	databases.
 ...
 *
 * @param string $log
 *        	the log to be recorded in the logs file.
 */
